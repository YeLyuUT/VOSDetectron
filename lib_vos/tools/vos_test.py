...
def im_detect_all(model, im, flo, box_proposals=None, timers=None, prev_cls_boxes = None):
    ...
    if cfg.TEST.BBOX_AUG.ENABLED:
        ...
        scores, boxes, im_scale, blob_conv = im_detect_bbox(
            model, im, flo, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, box_proposals)
    ...
    assert cls_keyps is None

    if cfg.TEST.NMS_WITH_MASK_IOU>0:
        cls_boxes, cls_segms = nms_with_mask_iou(
            cls_boxes, cls_segms, 
...
def im_conv_body_only(model, im, flo, target_scale, target_max_size):
    inputs, im_scale = _get_blobs(im, flo, None, target_scale, target_max_size)
...
def im_detect_bbox(model, im, flo, target_scale, target_max_size, boxes=None):
    """Prepare the bbox for testing"""
    inputs, im_scale = _get_blobs(im, flo, boxes, target_scale, target_max_size)
    ...
    if inputs['data_flow'] is None:
        inputs['data_flow'] = [None]
    else:
        inputs['data_flow'] = [torch.from_numpy(inputs['data_flow'])]
...
def iou_numpy(output, label):
    intersection = np.sum(output & label)
    union = np.sum(output | label)
    iou = (intersection) / (union + 1e-6)
    return iou

def iou_half_numpy(output, label):
    intersection = np.sum(output & label)
    union1 = np.sum(output)
    union2 = np.sum(label)
...
def nms_with_mask_iou(cls_boxes, cls_segms, iou_th = 0.9, max_per_class = 1):
    ...
    for i in range(len(sorted_inds)):
        ...
        for j in range(i+1,len(sorted_inds)):
            #iou = iou_numpy(masks[:,:,i], masks[:,:,j])
            #if iou>iou_th:
             #   discard = np.append(discard, j)
            iou1, iou2 = iou_half_numpy(masks[:,:,i], masks[:,:,j])            
            if iou1>iou_th or iou2>iou_th:
                discard = np.append(discard, j)
            
    keep = np.delete(np.arange(len(sorted_inds)), discard)
    masks = masks[:,:,keep]
    boxes = boxes[keep, :]
    classes = classes[keep]
    new_segms = []
    for i in range(len(keep)):
        rle = mask_util.encode(np.array(masks[:, :, i:i+1], order='F'))[0]
        rle['counts'] = rle['counts'].decode('ascii')
        new_segms.append(rle)
    ...
    for i in range(len(keep)):
        clss = classes[i]
        if len(cls_boxes[clss]) < max_per_class:
            cls_boxes[clss].append(boxes[i, :])
            cls_segms[clss].append(new_segms[i])
    return cls_boxes, cls_segms
...
def _get_blobs(im, flo, rois, target_scale, target_max_size):
    """Convert an image and RoIs within that image into network inputs."""
    blobs = {}
    blobs['data'], im_scale, blobs['im_info'] = \
        blob_utils.get_image_blob(im, target_scale, target_max_size)
    if flo is not None:
        blobs['data_flow'] = blob_utils.flo_to_blob(flo, target_scale, target_max_size)
    else:
        blobs['data_flow'] = None
    if rois is not None:
